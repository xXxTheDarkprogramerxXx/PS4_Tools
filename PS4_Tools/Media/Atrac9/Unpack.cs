...
namespace LibAtrac9
{
    internal static class Unpack
    {
        ...
        private static void ReadExtensionParams(BitReader reader, Block block)
        {
            ...
            {
                int a, b = 0;
                BandExtension.GetBexBandInfo(out bexBand, out a, out b, block.QuantizationUnitCount);
                if (block.BlockType == BlockType.Stereo)
                {
                    ReadHeader(block.Channels[1], reader, bexBand);
                }
                else
                {
                    reader.Position += 1;
            ...
            ReadHeader(block.Channels[0],reader,bexBand);

            block.BexDataLength = reader.ReadInt(5);
            if (block.BexDataLength <= 0) return;
            int bexDataEnd = reader.Position + block.BexDataLength;

            ReadData(block.Channels[0], reader, bexBand);

            if (block.BlockType == BlockType.Stereo)
            {
                ReadData(block.Channels[1], reader, bexBand);
            }
       ...
       static void ReadHeader(Channel channel, BitReader reader,int bexBand)
        {
        ...
        static void ReadData(Channel channel, BitReader reader, int bexBand)
        {
